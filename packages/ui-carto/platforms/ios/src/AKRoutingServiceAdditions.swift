...
class AKRoutingServiceAdditions: NSObject {
  ...
  public static func calculateRoute (_ service: NTRoutingService, _ request: NTRoutingRequest, _ profile: String,  _ stringifyResult:Bool, _ callback: @escaping (_ result: NTRoutingResult?, _ jsonStr: String?) -> Void) {
  ...
  public static func matchRoute (_ service: NTRoutingService, _ request: NTRouteMatchingRequest, _ profile: String, _ callback: @escaping (_ result: NTRouteMatchingResult?) -> Void) {
    DispatchQueue.global(qos: .background).async {
      service.setProfile(profile)
